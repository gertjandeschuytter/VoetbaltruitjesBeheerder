    ...
    public class KlantManager {
        ...
        public IReadOnlyList<Klant> ZoekKlanten(int? klantId, string? naam, string? adres)
        {
            ...
            try
            {
                if (klantId.HasValue)
                {
                    if (repo.bestaatKlant((int)klantId))
                    {
                        klanten.Add(repo.GeefKlant((int)klantId));
                    }
                }
                else
                {
            ...
            {
                throw new VoetbaltruitjeManagerException("zoekKlanten - " + ex.Message);
            }
        }
        ...
        public void verwijderKlant(Klant klant)
        {
            try
            {
                if (klant == null) throw new KlantManagerException("Klant is null!");
                if (!repo.bestaatKlant(klant)) throw new KlantManagerException("bestaat niet!");
                if (repo.heeftKlantBestellingen(klant)) throw new KlantManagerException("Klant heeft nog bestelling en kan niet verwijderd worden");
